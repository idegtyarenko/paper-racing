...
// ── Реалистичная физика («круг сцепления») ──────────────
// Режим physics: 'realistic' (см. Rules, candidates в turns.ts). Ход задаётся любым
// целым узлом внутри «круга сцепления» вокруг точки наката C = pos + vel: изменение
// скорости a = target − C ограничено по длине общим бюджетом сцепления, который
// делится между разгоном/торможением и поворотом. Отсюда: чем быстрее едешь, тем
// меньше можешь довернуть за ход (θ_max ≈ GRIP/|vel|) — появляется гоночная
// траектория (широкий заход → апекс → широкий выход). Значения тюнятся плейтестом.
/** Бюджет сцепления G: максимальная длина изменения скорости за ход (клетки/ход). */
export const REALISTIC_GRIP = 2;
/** Потолок разгона вперёд A_accel (клетки/ход): продольная составляющая a вдоль
 *  скорости не больше этого. Торможению назад доступен весь GRIP, поэтому болид
 *  тормозит быстрее, чем разгоняется. Он же — потолок разгона на старте (vel = 0). */
export const REALISTIC_ACCEL = 1;
/**
