// Русская локаль. Форма — из en.ts (тип Strings): все ключи и сигнатуры проверяются
// компилятором. Неразрывные пробелы ( ) расставляет scripts/typography.mjs —
// не правь их вручную — их поддерживает `npm run typo` (typo:check проверяет).

import type { Strings } from './en';

export const ru: Strings = {
  app: {
    title: 'Paper Racing',
  },

  editor: {
    step: {
      center: 'Нарисуй кольцевую трассу, не\u00A0отпуская палец.',
      adjust:
        'Потяни за\u00A0край дороги, чтобы поправить форму. ' +
        'Когда всё готово\u00A0— нажимай «Вперед».',
      finish:
        'Старт/финиш поставлен автоматически\u00A0— коснись трассы, чтобы передвинуть.',
      direction:
        'Направление выбрано\u00A0— коснись другой стрелки, чтобы развернуть, или продолжай.',
      ready: 'Трасса готова! Выбери режим гонки.',
    },
    stepTitle: {
      center: 'Трасса',
      adjust: 'Ширина',
      finish: 'Старт/финиш',
      direction: 'Направление',
      ready: 'Готово',
    },
    stepBadge: (n, total) => `Трасса: шаг ${n} из ${total}`,
    stepCounter: (n, total) => `Шаг ${n}/${total}`,
    errors: {
      selfCross: 'Трасса не должна пересекаться сама с собой — нарисуй снова.',
      tooSmall: 'Слишком короткая трасса — нарисуй круг побольше.',
      finishNarrow:
        'Слишком узкое место для старта — выбери другое или вернись и расширь.',
      finishMiss: 'Старт не может быть вне трассы.',
    },
    gestureCancelled: 'Сорвалось, попробуй еще раз.',
  },

  wizard: {
    title: 'Подготовка к гонке',
    stepMode: 'Режим',
    stepSetup: 'Настройки',
    resetWarn: 'Возврат на этот шаг сотрёт нарисованную трассу.',
    resetYes: 'Стереть и вернуться',
  },

  // Общее (бургер) меню — открывается из левого верхнего угла редактора.
  menu: {
    title: 'Меню',
    rules: 'Правила',
    join: 'Войти по\u00A0коду',
    language: 'Язык',
  },

  track: {
    strokeShort: 'Рисуй одним росчерком, не отпуская палец.',
    notClosed: 'Трасса должна быть кольцевой — замкни ее.',
    tooNarrow: 'Слишком узкая трасса — надо расширить.',
    noStartRoom:
      'На стартовой решетке негде выстроиться болидам — выбери место старта пошире.',
  },

  centerline: {
    selfOverlap: 'Слишком тесные повороты — не делай такие острые углы.',
  },

  players: {
    names: ['Красный', 'Синий', 'Зеленый', 'Оранжевый', 'Фиолетовый', 'Бирюзовый'],
    humansLabel: 'Люди',
    botsLabel: 'Боты',
    botsWithSeats: (seats: number): string => `Боты · свободно ${seats}`,
    difficultyLabel: 'Сложность ботов',
  },

  modeSelect: {
    title: 'С\u00A0кем играем?',
    local: 'С\u00A0друзьями на\u00A0одном устройстве',
    localSub: 'Передавайте телефон друг другу',
    online: 'С\u00A0друзьями онлайн',
    onlineSub: 'Создайте гонку и\u00A0пришлите друзьям код',
    ai: 'Против компьютера',
    aiSub: 'Гонка с\u00A0ботами: до\u00A0пяти соперников',
  },

  setup: {
    title: 'Настройка гонки',
    tabLineup: 'Состав',
    tabBehaviour: 'Поведение',
    tabRules: 'Правила',
    start: 'Поехали',
  },

  aiSelect: {
    easy: 'Новичок',
    medium: 'Любитель',
    hard: 'Профи',
    botsLabel: 'Количество ботов',
    difficultyLabel: 'Сложность',
  },

  settings: {
    title: 'Настройки заезда',
    helpLabel: 'Показать пояснение',
    tabDrive: 'Поведение машины',
    tabRules: 'Правила',
    driveModeSports: 'Спорткар',
    driveModeGt: 'GT',
    driveModeF1: 'F1',
    driveModeClassic: 'Классическое',
    driveModeCustom: 'Своё',
    driveExplainSports:
      'Машина тормозит быстрее, чем разгоняется, и\u00A0не\u00A0даёт одним ходом ' +
      'и\u00A0добавить газу, и\u00A0повернуть: в\u00A0поворот надо входить заранее. ' +
      'Получаем более реалистичные траектории.',
    driveExplainClassic:
      'На каждом ходу можно изменить скорость на 1 клетку ' +
      'по вертикали и горизонтали. Та самая игра из школьных тетрадок.',
    driveExplainGt:
      'Больше сцепления и\u00A0тормозов, чем у\u00A0спорткара, плюс лёгкий прижим\u00A0— ' +
      'быстрые повороты идут чуть острее.',
    driveExplainF1:
      'Сильный прижим: чем быстрее едешь, тем цепче держит. Быстрые повороты\u00A0— ' +
      'почти в\u00A0полный газ, медленные\u00A0— как обычно.',
    driveAccel: 'Разгон',
    driveBrake: 'Торможение',
    driveGrip: 'Маневренность',
    driveDownforce: 'Прижимная сила',
    lapsLabel: 'Кругов',
    lapsHint:
      'Сколько кругов длится гонка. Один круг\u00A0— короткая схватка, несколько дают ' +
      'время отыграться после ошибки.',
    lapsOption: (n: number): string => String(n),
    penaltyLabel: 'Штраф за вылет',
    penaltyHint:
      'Штрафные ходы за вылет с трассы. Можно наказывать по скорости ' +
      'вылета (чем быстрее вылетел — тем дольше выбираешься из гравия) или на фиксированное число ходов.',
    dynamic: 'Пропорционально скорости',
    static: 'Фиксированный',
    exponentLabel: 'Строгость',
    exponentHint: 'Насколько сильно растет штраф со скоростью вылета.',
    exponentStandard: 'Стандартная',
    exponentStrict: 'Высокая',
    staticTurnsLabel: 'Число ходов штрафа',
    staticTurnsHint: 'Сколько ходов болид простаивает в гравии после вылета.',
    reachableCells: 'Доступные клетки',
    turnLimitLabel: 'Время на ход',
    turnLimitHint:
      'Сколько дается на ход. Если игрок думает дольше лимита — остальные получают ' +
      'право пропустить его ход.',
    limit30s: '30 сек',
    limit1m: '1 мин',
    limit2m: '2 мин',
    limit5m: '5 мин',
    readOnlyNote: 'Это настройки создателя трассы — менять их\u00A0может только он.',
    done: 'Готово',
  },

  online: {
    namePlaceholder: 'Введи свое имя',
    joinByCode: 'Войти в онлайн-игру',
    joinTitle: 'Войти в онлайн-игру',
    codePlaceholder: 'Код игры',
    joinSubmit: 'Войти',
    lobbyBadge: 'Онлайн-игра',
    lobbyGuest: 'Ждем, пока создатель трассы начнет гонку…',
    roomCode: 'Код игры',
    trackLabel: 'Трасса',
    players: (n: number, max: number): string => `Гонщики · ${n}/${max}`,
    hostBadge: 'Создатель',
    noGuests: 'Поделись кодом игры\u00A0— пока никто не\u00A0подключился',
    enterName: 'Введи имя, чтобы продолжить',
    leaveLobby: 'Выйти из\u00A0игры',
    cancelRace: 'Отменить игру',
    cancelConfirm: 'Отменить игру? Остальные вылетят из\u00A0неё.',
    copied: 'Ссылка скопирована',
    codeCopied: 'Код скопирован',
    waiting: 'Ждем еще хотя бы одного гонщика…',
    you: 'ты',
    yourTurn: 'Твой ход: выбери направление и жми «Едем!»',
    turnOf: (name) => `Ходит ${name}. Ждем…`,
    skippable: (name) =>
      `${name} долго не ходит. Можно пропустить ход — болид проедет прямо.`,
    skipTurnBtn: 'Пропустить ход',
    offline: 'не в сети',
    notFound: 'Игра с таким кодом не найдена.',
    full: 'В гонке уже максимум участников (6).',
    started: 'Эта гонка уже началась.',
    error: 'Не удалось подключиться. Попробуй еще раз.',
    closed: 'Игра завершена или закрыта создателем трассы.',
    sending: 'Отправляю ход…',
    sendFailed: 'Не получилось отправить ход. Проверь связь и попробуй еще раз.',
    retrySend: 'Отправить снова',
    joining: 'Подключаемся…',
    starting: 'Запускаем…',
    startFailed: 'Не получилось начать гонку. Попробуй еще раз.',
    rematchWaiting:
      'Создатель трассы может запустить рематч — тогда все продолжат на этой же трассе.',
    leaveRace: 'Выйти из\u00A0игры',
    leaveBotsConfirm: 'Если ты\u00A0выйдешь, у\u00A0остальных гонка прервётся.',
    hostLeftStalled: 'Создатель трассы вышел из\u00A0игры. Эту гонку не\u00A0продолжить.',
    hostOffline:
      'Создатель трассы не\u00A0в\u00A0сети. Боты не\u00A0поедут, пока он\u00A0не\u00A0вернётся.',
    hostLeftNoRematch:
      'Создатель трассы вышел из\u00A0игры\u00A0— рематча не\u00A0будет.',
    playerLeft: (name) => `${name} покинул игру.`,
    playerRetired: (name) => `${name} сошел с\u00A0дистанции.`,
    reconnecting: 'Нет связи. Переподключаемся…',
    raceCode: (code) => `Код игры ${code}`,
    resumeTitle: (code) => `Вернуться в игру ${code}?`,
    resumeYes: 'Вернуться в игру',
    setupChanged: (what: string): string =>
      `Создатель трассы изменил настройки: ${what}.`,
    setupChangedMany: 'Создатель трассы изменил настройки гонки.',
    gameGone: 'Игра больше недоступна.',
  },

  race: {
    driver: (name) => `Ходит ${name}.`,
    driverYou: 'Твой ход.',
    you: 'Я',
    rivalsMoving: 'Ходят соперники…',
    hintPick: 'Выбери точку и подтверди, нажав «Едем!»',
    finalWarn: ' Кто\u00A0пересечет черту раньше, тот и\u00A0впереди.',
    classification: 'Классификация',
    upNext: 'Очередь ходов',
    lapOf: (lap: number, total: number): string => `Круг ${lap}/${total}`,
    speedUnit: 'км/ч',
    stalled: 'Не ходит',
    finished: (place) => `Финиш · ${place}-й`,
    moves: (n) => {
      const t = n % 100 > 10 && n % 100 < 20 ? 0 : n % 10;
      return `${n} ${t === 1 ? 'ход' : t > 1 && t < 5 ? 'хода' : 'ходов'}`;
    },
    retired: 'Сошел',
    raceComplete: 'Гонка завершена',
    youWon: 'Ты\u00A0победил',
    youWonSub: 'Первое место — отличный заезд',
    someoneWon: (name) => `${name} на\u00A01-м месте`,
    someoneWonSub: 'В следующий раз повезет',
    podiumSub: (place) => `${place}-е место\u00A0— достойный результат`,
    hotseatWonSub: 'Поздравляем победителя!',
    allRetired: 'Все сошли',
    allRetiredSub: 'Никто не финишировал — результата нет',
    drawTitle: 'Фотофиниш',
    draw: 'Ого, фотофиниш не смог выявить победителя!',
    retire: 'Сойти с дистанции',
    retireConfirmTitle: 'Сойти с дистанции?',
    retireConfirmYes: 'Сойти',
    standingsLabel: 'Текущие места',
    queueLabel: 'Порядок ходов',
    preselectCleared: 'Предварительный выбор хода сброшен',
    updated: 'Обновлено до последней версии',
  },

  buttons: {
    next: 'Вперед',
    back: 'Назад',
    redraw: 'Перерисовать',
    chooseMode: 'Выбрать режим',
    confirmMove: 'Едем!',
    sameTrack: 'Рематч',
    replay: 'Повтор',
    closeReplay: 'Закрыть повтор',
    sameTrackNewMode: 'Та же трасса, другой состав',
    newTrack: 'Нарисовать новую',
    cancel: 'Отмена',
    rulesTitle: 'Правила',
    toWheel: 'Всё понятно',
  },

  rules: {
    body:
      'Paper Racing\u00A0— гонка по\u00A0клеточкам. Нарисуй кольцевую трассу и\u00A0обгони на\u00A0ней соперников.\n\n' +
      'Болид катится по\u00A0инерции\u00A0— сам едет дальше в\u00A0ту\u00A0же\u00A0сторону. За\u00A0ход его можно лишь чуть подправить: выбери одну из\u00A0подсвеченных клеток. Резко не\u00A0свернешь и\u00A0сразу не\u00A0остановишься\u00A0— тормози и\u00A0заходи в\u00A0повороты заранее.\n\n' +
      'Вылетел с\u00A0трассы\u00A0— увяз в\u00A0гравии. Чем быстрее летел, тем дольше выбираться, а\u00A0выбравшись\u00A0— снова трогаешься с\u00A0места.\n\n' +
      'Сквозь чужой болид не\u00A0проехать и\u00A0на\u00A0его клетку не\u00A0встать: соперники держат позицию, как в\u00A0настоящей гонке.\n\n' +
      'Победит тот, кто первым проедет все круги. А\u00A0если несколько болидов финишируют на\u00A0одном ходу\u00A0— впереди тот, чей болид пересек черту раньше внутри хода.\n\n' +
      'Надоело ехать? Из\u00A0гонки можно выйти в\u00A0любой момент и\u00A0досмотреть ее\u00A0со\u00A0стороны.\n\n' +
      'Поведение машины и\u00A0правила гонки можно подстроить под себя, когда трасса нарисована,\u00A0— ищи вкладки «Поведение» и\u00A0«Правила».',
  },

  install: {
    title: 'Установи игру на домашний экран',
    body: 'Откроется на весь экран и работает без сети.',
    action: 'Установить',
    iosBefore: 'Нажми ',
    iosMid: ' внизу, потом ',
    iosAfter: ' «Поделиться» → «На экран „Домой“».',
    close: 'Закрыть',
  },
};
